...
export default function Pricing() {
  ...
  // UPI ID — apna UPI ID yahan daalo
  const UPI_ID = "rajvardhan@upi"
  const UPI_NAME = "MediCare"

  const getUPILink = (amount) => {
    return `upi://pay?pa=${UPI_ID}&pn=${UPI_NAME}&am=${amount}&cu=INR&tn=MediCare ${selectedPlan?.name} Plan`
  }
          ...
          <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl">
            {!paymentDone ? (
              <>
                ...
                {/* UPI QR Code Section */}
                <div className="bg-gradient-to-br from-teal-50 to-cyan-50 rounded-2xl p-6 mb-6 text-center border border-teal-100">
                  <p className="text-gray-600 font-medium mb-3">Scan QR Code to Pay</p>
                  {/* QR Code - UPI link */}
                  <div className="w-40 h-40 bg-white rounded-xl mx-auto mb-3 flex items-center justify-center border-2 border-teal-200 overflow-hidden">
                    <img
                      src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(getUPILink(selectedPlan.price))}`}
                      alt="UPI QR Code"
                      className="w-full h-full"
                    />
                  </div>
                  <p className="text-sm text-gray-500 mb-1">UPI ID:</p>
                  <p className="font-bold text-teal-600 text-lg">{UPI_ID}</p>
                  <p className="text-xs text-gray-400 mt-1">Amount: ₹{selectedPlan.price}</p>
                </div>

                {/* UPI Apps */}
                <div className="mb-6">
                  <p className="text-center text-gray-500 text-sm mb-3">Or pay with UPI app</p>
                  <a href={getUPILink(selectedPlan.price)}
                    className="block w-full bg-gradient-to-r from-teal-500 to-cyan-500 text-white py-3 rounded-xl font-bold text-center hover:shadow-lg transition-all mb-2">
                    📱 Open UPI App to Pay
                  </a>
                  <div className="grid grid-cols-4 gap-2 mt-3">
                    {[
                      { name: 'GPay', icon: '🟢', link: `gpay://upi/pay?pa=${UPI_ID}&pn=${UPI_NAME}&am=${selectedPlan.price}&cu=INR` },
                      { name: 'PhonePe', icon: '🟣', link: `phonepe://pay?pa=${UPI_ID}&pn=${UPI_NAME}&am=${selectedPlan.price}&cu=INR` },
                      { name: 'Paytm', icon: '🔵', link: `paytmmp://pay?pa=${UPI_ID}&pn=${UPI_NAME}&am=${selectedPlan.price}&cu=INR` },
                      { name: 'BHIM', icon: '🟠', link: getUPILink(selectedPlan.price) },
                    ].map((app) => (
                ...
                <div className="bg-yellow-50 rounded-xl p-3 mb-4 text-center">
                  <p className="text-yellow-700 text-xs">⚠️ After payment, click "I've Paid" button below</p>
                </div>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-teal-600 font-medium">Redirecting to dashboard...</p>
